...
# Ensure models can be imported correctly
...
async def upgrade(db, session=None):
    ...
    if "Audit_Logs" not in existing:
        ...
        await db["Audit_Logs"].create_index("activity_id", unique=True)
    ...
    if "Ledger_Audit_Logs" not in existing:
        await db.create_collection(
            "Ledger_Audit_Logs",
            validator={"$jsonSchema": clean_json_schema(LedgerAuditLog.model_json_schema())},
        )
        await db["Ledger_Audit_Logs"].create_index("activity_id", unique=True)
    ...
    if "Demand_Audit_Logs" not in existing:
        await db.create_collection(
            "Demand_Audit_Logs",
            validator={"$jsonSchema": clean_json_schema(DemandAuditLog.model_json_schema())},
        )
        await db["Demand_Audit_Logs"].create_index("activity_id", unique=True)
    ...
    if "Stock_Movement_Logs" not in existing:
        await db.create_collection(
            "Stock_Movement_Logs",
            validator={"$jsonSchema": clean_json_schema(StockMovementLog.model_json_schema())},
        )
        await db["Stock_Movement_Logs"].create_index("activity_id", unique=True)
    ...
    if "Part_Audit_Logs" not in existing:
        await db.create_collection(
            "Part_Audit_Logs",
            validator={"$jsonSchema": clean_json_schema(PartAuditLog.model_json_schema())},
        )
        await db["Part_Audit_Logs"].create_index("activity_id", unique=True)
